...
import sys
sys.path.append('../')
from baselines import DifferentiableSelector, ConcreteMask2d
...
if __name__ == '__main__':
    ...
    for trial in range(args.num_trials):
        ...
        for num in num_features:
            ...
            selector_layer = ConcreteMask2d(width=8, patch_size=4, num_select=num)
            ...
            inds = np.sort(logits.argmax(axis=1))
            if len(np.unique(inds)) != num:
                print(f'{len(np.unique(inds))} selected instead of {num}, appending extras')
                num_extras = num - len(np.unique(inds))
                remaining = np.setdiff1d(np.arange(64), inds)
                inds = np.sort(np.concatenate([np.unique(inds), remaining[:num_extras]]))
            mask = torch.from_numpy(np.isin(np.arange(64), inds)).reshape(8, 8).float()
        ...
        # Save results.
        with open(f'results/cifar_cae_{trial}.pkl', 'wb') as f:
